...
// BSD Lincese.
// ---------------------------------------------------------------------------
// File: unit_test_nvgpu_omp.cuh
// Unit test for OpenMP GPU state-vector simulation with NVIDIA GPU backend.
// ---------------------------------------------------------------------------
        ...
        ValType* sv_imag_expected)
{
    ...
    {
        if ( abs(sv_real_expected[i] - sim.sv_real_cpu[i]) > ERROR_BAR
                || abs(sv_imag_expected[i] - sim.sv_imag_cpu[i]) > ERROR_BAR)
